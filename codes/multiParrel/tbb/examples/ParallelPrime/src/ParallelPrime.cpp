...
//! Grainsize parameter // someday we should
                        // convert program to use auto_partitioner
...
class Multiples {
    ...
    //! Indexes into window
    /** my_striker[k] is an index into
        my_composite corresponding to
        an odd multiple of my_factor[k]. */
    Number* my_striker;
...
public:
    ...
    //! Find primes in range [start,window_size),
    //  advancing my_striker as we go.
    /** Returns number of primes found. */
    Number find_primes_in_window( Number start, 
                                  Number window_size ) {
        bool* is_composite = my_is_composite;
        memset( is_composite, 0, window_size/2 );
        for( size_t k=0; k<n_factor; ++k )
            my_striker[k] = strike( my_striker[k]-m/2, 
                                    window_size/2, 
                                    my_factor[k] );
        Number count = 0;
...
//
// Actually very simple creation of a
// range specialized for the Sieve
//
class SieveRange {
...
public:
    ...
    // should we split? (is it worth the overhead?)
        ...
        my_end(r.my_end)
    {
        ...
        Number middle = r.my_begin +
                        (r.my_end-r.my_begin+r.my_stride-1)/2;
...
class Sieve {
public:
    //! Prime multiples to consider, and 
    //  working storage for this subsieve.
    Multiples multiples;
...
Number ParallelCountPrimes( Number n ) {
    ...
    if( n>=3 ) {
        ...
        if( PrintPrimes ) 
            printf("---\n");
        parallel_reduce( SieveRange( s.multiples.m, n, 
                                     s.multiples.m, GrainSize ),
                         s );
        count += s.count;
    }
    return count;
...
static Number ParseCommandLine( int argc, char* argv[] ) {
    ...
    if( i<argc && !isdigit(argv[i][0]) ) { 
        ...
        fprintf(stderr,
                "Usage: %s [['pause'] n [nthread [grainsize]]]\n",
                argv[0]);
        fprintf(stderr,"where n is a positive integer [%lu]\n",n);
        fprintf(stderr,
                "%s the form low:high [%ld:%lu]\n",
                "\tnthread is a non-negative integer, or range of",
                NThread.low,NThread.high);
        fprintf(stderr,
                "\tgrainsize is an optional positive integer [%lu]\n",
                GrainSize);
        exit(1);
    }
    if( i<argc )
...
int main( int argc, char* argv[] ) {
    ...
    for( Number p=NThread.low; p<=NThread.high; ++p ) {
		...
		// Added missing parenthesis to the book version
